...
The (normalized) energy of the membrane is
\begin{equation}
E(t)=\frac12\int_{0}^{1}(u_t^2 + c^2 u_x^2)\,dx.
\end{equation}
The first term yields the kinetic energy and the second term
...
You must put your implementation of the forbidden functions in the file
...
\subsection{ Gaussian patch}
You would use:
\[ \psi(x,y) = \frac{1}{2\pi\sigma^2} e^{-\frac{1}{2\sigma^2} \|(x,y)-(x_0,y_0)\|^2}. \]
This would be handled by the extra parameter to the model: \( \sigma \).
Note that the integral of \(\psi\) is not 1, but as \(\sigma\to 0\) this is
a proper approximation to the \(\delta\)-function patch.
...
parameter \(\gamma\) and/or \(\sigma\) to your model if you implement
this optional extension.  Autograder will call your
\texttt{build\_model}. Of course, you must adapt other functions that
use the model, so that they properly handle the new parameter.
